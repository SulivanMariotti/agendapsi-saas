...
export default function ProfessionalScheduleSettingsClient({ initialSchedule }) {
  ...
  return (
    <div className="mx-auto max-w-5xl p-3 sm:p-6 text-sm">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-extrabold text-slate-400">Profissional</p>
          <h1 className="text-xl font-extrabold tracking-tight text-slate-900">Configuração da agenda</h1>
          <p className="text-xs text-slate-500">Defina sua grade (intervalo), horários por dia, buffer e almoço.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="secondary" icon={ArrowLeft} onClick={() => router.push("/profissional")}>Voltar</Button>
          <Button variant="primary" icon={Save} onClick={save} disabled={busy}>Salvar</Button>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="rounded-2xl border border-slate-100 bg-slate-50 p-3">
                ...
                <div className="mt-2 text-xs text-slate-600 space-y-1">
                  <p>Intervalo: <span className="font-mono font-bold">{slotIntervalMin}m</span></p>
                  <p>Duração padrão: <span className="font-mono font-bold">{defaultDurationMin}m</span></p>
                  <p>Buffer: <span className="font-mono font-bold">{bufferMin}m</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-[11px] text-slate-400">Se ativa, a pausa é aplicada aos dias habilitados (quando estiver dentro do horário do dia).</p>
                </div>
        ...
        <Card title="Horário de atendimento por dia">
              ...
              return (
                <div key={d.key} className="rounded-2xl border border-slate-100 bg-white p-3">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[11px] font-bold text-slate-500">Início</label>
                      <input
                        type="time"
                        step={timeStepSec}
                        className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"
                    ...
                    <div>
                      <label className="text-[11px] font-bold text-slate-500">Fim</label>
                      <input
                        type="time"
                        step={timeStepSec}
                        className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"
          ...
          <div className="mt-4 rounded-2xl border border-slate-100 bg-slate-50 p-3">
            <p className="text-[11px] font-bold text-slate-500">Nota</p>
            <p className="text-[11px] text-slate-500">
              A agenda (visão Dia) passa a mostrar apenas os horários abertos do dia, respeitando a pausa de almoço.
            </p>
          </div>
        </Card>
      </div>
