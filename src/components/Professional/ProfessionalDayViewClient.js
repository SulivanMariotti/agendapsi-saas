                ...
                return (
                  ...
                  >
                    ...
                    {occ ? (
                      occ.isBlock ? (
                        <div
                          className={`text-left rounded-xl border border-l-4 overflow-hidden ${statusCardSoftClass(
                            occ.status
                          )} ${statusAccentBorderClass(occ.status)} ${
                        ...
                        <button
                          ...
                          className={`w-full text-left rounded-xl border border-l-4 overflow-hidden hover:brightness-[0.99] transition ${statusCardSoftClass(
                            occ.status
                          )} ${statusAccentBorderClass(occ.status)} ${
                            ...
                            <div className="shrink-0 flex flex-col items-end text-right">
                              <div className="flex items-center justify-end gap-1">
                                ...
                                <span
                                  ...
                                  className={`inline-flex items-center justify-center ${statusIconColorClass(occ.status)}`}
                                >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs"
                value={replicateDays}
                onChange={(e) => setReplicateDays(parseInt(e.target.value, 10) || 0)}
              />
              <p className="mt-1 text-[11px] text-slate-400">Máximo 15 dias.</p>
            </div>
            <div className="flex items-end">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 w-full">
                <p className="text-xs text-slate-500 font-bold">Dica</p>
                <p className="text-xs text-slate-600">Use reserva para segurar horário antes de vincular paciente.</p>
              </div>
            </div>
          </div>
            ...
            >
              {Array.from({ length: 8 }).map((_, i) => {
                ...
                <option key={b} value={b}>
                  {b} bloco{b > 1 ? "s" : ""} ({b * (slotIntervalMin || 30)} min)
                </option>
                );
              })}
            </select>
...
<div className="flex gap-2 pt-1">
          ...
          <Button variant="primary" onClick={submitAppointment} disabled={busy} className="flex-1">
            Agendar
          </Button>
        </div>
      </div>
