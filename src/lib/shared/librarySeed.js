/**
 * Artigos base (seed) para a Biblioteca.
 *
 * Uso:
 * - Fallback no painel do paciente quando ainda não há artigos publicados.
 * - Seed opcional no Admin (criar artigos modelo no Firestore).
 *
 * Importante (clínico): leitura NÃO substitui a sessão.
 */

export const LIBRARY_SEED_ARTICLES = [
  {
    id: "constancia",
    category: "Constância",
    readingTime: "2–3 min",
...
export function seedArticleToDoc(a) {
  const paragraphs = Array.isArray(a?.body) ? a.body : [];
  const content = paragraphs.join("\n\n");
  const categoryLabel = String(a?.category || "").trim() || "Geral";
  ...
  return {
    ...
    pinned: false,
    order: 100,
    readingTime: String(a?.readingTime || "").trim() || null,
    seed: true,
  };
}
