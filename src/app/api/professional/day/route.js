...
export async function GET(request) {
  const auth = await requireProfessionalApi(request, { bucket: "professional:day", limit: 240, windowMs: 60_000 });
  ...
  const data = await getProfessionalDayData({ tenantId: session.tenantId, isoDate });
