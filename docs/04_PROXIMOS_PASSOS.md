...
## F) ANÁLISE FAT (Admin-only — BI operacional)
- [x] Rota dedicada **`/admin/fat`** (não aparece no menu do `/admin`).
- [x] Parser NFS-e (SPED) com normalização numérica (ponto/vírgula).
...
- [ ] Melhorias futuras (quando estabilizar):
  ...
  - Export CSV completo (histórico) + resumo por mês.
  - Exclusão com “trava” por Emitente/competência (segurança extra).
